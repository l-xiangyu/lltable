...
import { useJbsReferenceQueryResult } from '../jbs-reference/use-jbs-reference-query-result';
...
export const GroupPointProvider: FC<GroupPointProviderProps> = ({ children, query }) => {
  ...
  const jbsQueryResult = useJbsReferenceQueryResult(groupPointQuery);
  const { data: resGroupPoints } = useQuery({
    ...
    // 引用表分组在内存中计算，不走 Teable 聚合接口
    enabled:
      Boolean(tableId && isHydrated && groupBy?.length) &&
      visible &&
  ...
  const groupPoints = useMemo(() => {
    if (jbsReference.isReferenceTable) {
      return jbsQueryResult?.groupPoints ?? null;
    }
    return resGroupPoints || null;
  }, [jbsReference.isReferenceTable, jbsQueryResult, resGroupPoints]);
