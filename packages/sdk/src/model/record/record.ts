...
import { getJbsReferenceBridgeHandlers } from '../../context/jbs-reference/jbs-reference-bridge';
...
export class Record extends RecordCore {
  ...
  async updateCell(
    ...
    // 引用表：无 ShareDB doc，改走嵌入层注册的主端保存桥接
    if (!this.doc) {
      const bridge = getJbsReferenceBridgeHandlers();
      if (!bridge?.saveCellUpdate) {
        return;
      }
      const oldCellValue = this.fields[fieldId];
      const normalized = cellValue === undefined ? null : cellValue;
      try {
        this.fields[fieldId] = normalized;
        await bridge.saveCellUpdate({
          recordId: this.id,
          fieldId,
        ...
        this.fields[fieldId] = oldCellValue;
