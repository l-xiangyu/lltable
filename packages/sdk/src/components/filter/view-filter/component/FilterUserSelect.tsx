...
import { FieldType, isMeTag, Me } from '@teable/core';
...
const JBS_USER_FIELD_TYPES = [FieldType.User, FieldType.CreatedBy, FieldType.LastModifiedBy];
...
const FilterUserSelect = (props: IFilterUserProps) => {
  ...
  const useJbsUsers = jbsUserField.enabled && JBS_USER_FIELD_TYPES.includes(field.type);
  ...
  const { data: collaboratorsData = defaultData } = useQuery({
    ...
    enabled: !isTemplate && !useJbsUsers,
  });
  ...
  const jbsFlatData = useMemo(() => {
    if (!useJbsUsers) {
      return undefined;
    }
    const keyword = search.trim().toLowerCase();
    const list = jbsUserField.flatCollaborators;
    if (!keyword) {
      return list;
    }
    return list.filter(
  ...
  }, [useJbsUsers, jbsUserField.flatCollaborators, search]);

  const data = useJbsUsers
    ? jbsFlatData
    : isTemplate
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    if (useJbsUsers) {
      jbsUserField.onSearch(value);
    }
  };

  const mapJbsFilterValue = (value: string[] | string | null): string[] | string | null => {
    if (!useJbsUsers || !jbsUserField.currentJbsUserId) {
      return value;
    }
    const mapOne = (item: string) => (isMeTag(item) ? jbsUserField.currentJbsUserId! : item);
  ...
  const handleSelect = (value: string[] | string | null) => {
    onSelect(mapJbsFilterValue(value));
  };

  return (
    <FilterUserSelectBase {...props} data={data} onSearch={handleSearch} onSelect={handleSelect} />
  );
};
