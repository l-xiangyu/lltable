...
import {
  ...
  isJbsReferenceFieldEditable,
} from '../../../context/jbs-reference';
  ...
  return useCallback(
    (fields: IFieldInstance[]) =>
      (
        ...
        // 引用表字典：options 异步同步时校验会失败，保留主端返回值避免单元格空白
        if (
          jbsReference.isReferenceTable &&
          type === FieldType.SingleSelect &&
          !validateCellValue.success &&
          cellValue != null &&
          cellValue !== ''
        ) {
          cellValue = typeof cellValue === 'string' ? cellValue : String(cellValue);
        } else if (
          jbsReference.isReferenceTable &&
          type === FieldType.User &&
          !validateCellValue.success &&
          ...
          // 引用表用户：preview 可能为 userId 字符串，转成 User 单元格结构
          cellValue =
            typeof cellValue === 'string'
              ? { id: cellValue, title: cellValue }
              : validateCellValue.data ?? cellValue;
        } else {
          cellValue = validateCellValue.success ? validateCellValue.data : undefined;
        }
        const recordReadOnly = !recordEditable && !isPrefilling;
        // 引用表新增行：仍按主端 isEditable 锁定不可编辑列
        const jbsFieldNotEditable =
          jbsReference.isReferenceTable && !isJbsReferenceFieldEditable(jbsReference, fieldId);
        const fieldLocked = isPrefilling
          ? jbsFieldNotEditable
          : record.isLocked(fieldId) || jbsFieldNotEditable;
        ...
        switch (type) {
          case FieldType.SingleLineText: {
            // 玲珑表格下钻字段：展示主端 preview 值，编辑时改参数字段
            if (jbsDrillField.enabled && jbsDrillField.isDrillField(fieldId)) {
              const display = field.cellValue2String(cellValue) || '';
              const drillMeta = jbsDrillField.getDrillFieldMeta(fieldId);
              return {
                ...baseCellProps,
                type: CellType.Select,
                data: display ? [{ id: record.id, title: display }] : [],
                displayData: display ? [display] : [],
                choiceSorted: [],
                isMultiple: false,
                allowPreviewWhenReadonly: true,
                onPreview: () => {
                  if (drillMeta) {
                    jbsDrillField.openDrillFieldDetail?.({
                      drillMeta,
                ...
                customEditor: (editorProps, editorRef) => (
                  <GridJbsDrillFieldEditor
                    ref={editorRef}
                    field={field}
                    record={record}
          ...
          case FieldType.SingleSelect: {
            ...
            const dictDisplayChoices =
              jbsReference.isReferenceTable && jbsReference.isDictField?.(fieldId)
                ? jbsReference.getDictDisplayChoices?.(fieldId)
                : undefined;
            const choiceSorted = dictDisplayChoices?.length
              ? dictDisplayChoices
              : field.options.choices;
            const choiceMap =
              dictDisplayChoices?.length && dictDisplayChoices
                ? Object.fromEntries(
                    dictDisplayChoices.map(({ id, name, color }) => [
                      name,
                      {
                        id,
                        name,
                        color: ColorUtils.shouldUseLightTextOnColor(color)
                          ? colors.white
                          : colors.black,
          ...
          case FieldType.LastModifiedBy: {
            ...
            const data = cv.map((item) => {
              const resolved = resolveJbsUserCellValueForDisplay(
                item as IUserCellValue,
                jbsUserField
              );
              const { title, avatarUrl } = resolved;
              return {
                ...resolved,
                name: title,
                // IUserData.avatarUrl 不接受 null
    ...
    [
      i18nMap,
      jbsDrillField,
      jbsReference.dictOptionsVersion,
      jbsReference.getDictDisplayChoices,
      jbsReference.isDictField,
      jbsReference.isReferenceTable,
      jbsUserField,
      jbsUserField.displayCacheVersion,
      recordEditable,
      resolvedTheme,
      rowHeight,
      t,
