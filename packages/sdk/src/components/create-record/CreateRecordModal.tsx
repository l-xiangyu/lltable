...
import { useJbsReferenceRecords, isJbsReferenceFieldEditable } from '../../context/jbs-reference';
import {
  useBaseId,
  useFields,
  useRecordOperations,
...
export const CreateRecordModal = (props: ICreateRecordModalProps) => {
  ...
  const canCreateField = useCallback(
    (field: IFieldInstance) => {
      if (!field.canCreateFieldRecord) {
        return false;
      }
      return isJbsReferenceFieldEditable(jbsReference, field.id);
    },
    [jbsReference]
  );

  const showFieldsId = useMemo(
    () => new Set(showFields.filter(canCreateField).map((field) => field.id)),
    [showFields, canCreateField]
  );

  const fields = useMemo(
    () =>
      viewId
        ? allFields.filter(canCreateField).filter((field) => showFieldsId.has(field.id))
        : allFields.filter(canCreateField),
    [allFields, canCreateField, showFieldsId, viewId]
  );

  const hiddenFields = useMemo(
    () =>
      viewId ? allFields.filter(canCreateField).filter((field) => !showFieldsId.has(field.id)) : [],
    [allFields, canCreateField, showFieldsId, viewId]
  );
