...
export const autoFillCellVoSchema = z.object({
  taskId: z.string(),
  /** When taskId is 'sync', the generated cell value is returned so UI can show it immediately */
...
export const AutoFillCellRoute: RouteConfig = registerRoute({
  ...
  responses: {
    200: {
      description: 'Returns taskId (sync = done in request) and optional value when sync',
      content: {
        'application/json': {
