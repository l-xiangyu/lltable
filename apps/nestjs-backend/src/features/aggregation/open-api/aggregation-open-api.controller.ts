...
} from '../../record/open-api/ai-auto-fill.constants';
...
export class AggregationOpenApiController {
  ...
  async getTaskStatusCollection(
    ...
    // 中文注释: 仅返回“进行中”的任务，完成/失败的任务不再占用 UI 轮询展示
    ...
    const tasks = await this.prismaService.txClient().task.findMany({
      ...
      select: { id: true, snapshot: true, createdTime: true },
      orderBy: { createdTime: 'desc' },
      take: 200,
    });

    const cells: Array<{ recordId: string; fieldId: string }> = [];
    const fieldMap: Record<string, { taskId: string; completedCount: number; totalCount: number }> =
      {};

    const seenCell = new Set<string>();
    for (const t of tasks) {
      let snap: IAiAutoFillTaskSnapshot | null = null;
      try {
        snap = t.snapshot ? (JSON.parse(t.snapshot) as IAiAutoFillTaskSnapshot) : null;
      } catch {
        snap = null;
      }
      if (!snap?.recordId || !snap?.fieldId) continue;

      const cellKey = `${snap.recordId}:${snap.fieldId}`;
      if (!seenCell.has(cellKey)) {
        seenCell.add(cellKey);
        cells.push({ recordId: snap.recordId, fieldId: snap.fieldId });
      }

      // 中文注释: fieldMap 用“最新任务”覆盖（前端按 fieldId 显示进度条）
      if (!fieldMap[snap.fieldId]) {
        fieldMap[snap.fieldId] = {
          taskId: t.id,
          completedCount: 0,
          totalCount: 1,
        };
