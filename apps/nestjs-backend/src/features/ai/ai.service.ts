...
export class AiService {
  ...
  /**
   * 中文注释: 官网做法——把附件直接发给大模型（URL 或 base64），由模型识别文档/图片，无需服务端解析
   * 当 fileParts 为空时退化为纯文本 generateText
   */
  async generateTextWithAttachments(
    ...
    options: {
      ...
      modelKey?: string;
      task?: Task;
      fileParts: Array<{ data: string; mediaType: string }>;
    }
  ): Promise<string> {
    const { prompt, fileParts, modelKey, task = Task.Coding } = options;
    ...
    const parts: FilePart[] = fileParts.map(({ data, mediaType }) => ({
      type: 'file' as const,
      data,
      mediaType,
    }));
    const { text } = await generateText({
      model: modelInstance,
      messages: [{ role: 'user', content: [textPart, ...parts] }],
      temperature: 0,
    });
    return text;
  }
