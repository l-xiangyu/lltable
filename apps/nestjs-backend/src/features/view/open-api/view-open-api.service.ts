...
export class ViewOpenApiService {
  ...
  async updateViewColumnMeta(
     ...
     */
    const isJbsReferenceTable = this.isJbsReferenceTableDescription(view.table.description);
    if (
      isHiddenPrimaryField &&
      !isJbsReferenceTable &&
      !allowHiddenPrimaryType.includes(view.type as ViewType)
    ) {
      throw new CustomHttpException(
