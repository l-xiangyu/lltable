...
export class FieldService implements IReadonlyAdapterService {
  ...
  private isJbsReferenceTableDescription(description?: string | null) {
    if (!description?.startsWith('JBS_META:')) {
      return false;
    }
    try {
      const parsed = JSON.parse(description.slice('JBS_META:'.length)) as {
        jbsTableType?: string;
        readonly?: boolean;
      };
      return parsed.jbsTableType === 'reference' && parsed.readonly === true;
    } catch {
      return false;
    }
  }
  ...
  private async handleFieldProperty(
    ...
    if (key === 'unique' || key === 'notNull') {
      let skipPhysicalValidation = false;
      if (key === 'notNull') {
        const tableMeta = await this.prismaService.txClient().tableMeta.findUnique({
          where: { id: tableId },
          select: { description: true },
        });
        // 引用表数据不落 Teable 业务表，必填只作为字段元数据与前端校验，不修改物理表约束。
        skipPhysicalValidation = this.isJbsReferenceTableDescription(tableMeta?.description);
      }
      if (!skipPhysicalValidation) {
