...
export class UserService {
  ...
  async createUserWithSettingCheck(
    ...
    skipSignUpCheck: boolean = false
  ) {
    const setting = await this.settingService.getSetting();
    // OAuth/SSO 用户应绕过disallow SignUp检查，因为它们由SSO提供程序管理
    if (!skipSignUpCheck && setting?.disallowSignUp) {
    ...
    // OAuth/SSO 注册用户不赋予实例超级管理员，仅普通注册可成为首个管理员
    const assignAdminIfFirstUser = !skipSignUpCheck;
    return await this.createUser(
      user,
      account,
      defaultSpaceName,
      autoSpaceCreation,
      assignAdminIfFirstUser
    );
  }
  ...
  /**
   * @param assignAdminIfFirstUser 是否在“当前实例首个用户”时赋予超级管理员。
   * 为 false 时（如 OIDC/主端 byProvider 创建的用户）永不赋权，避免多租户下普通用户被误设为管理员。
   */
  async createUser(
    user: Omit<Prisma.UserCreateInput, 'name'> & { name?: string },
    account?: Omit<Prisma.AccountUncheckedCreateInput, 'userId'>,
    defaultSpaceName?: string,
    autoSpaceCreation: boolean = true,
    assignAdminIfFirstUser: boolean = true
  ) {
    ...
    const isAdmin = assignAdminIfFirstUser && userTotalCount === 0;
  ...
  /**
   * 按 (provider, providerId) 查找用户，不存在则创建用户并关联 account。
   * 供 OpenAPI POST /api/admin/users/byProvider 使用（主端 OIDC 登录时初始化用户）。
   * 与 OIDC 的 findOrCreateUser 使用相同的 account 匹配逻辑，
   * 确保主端初始化创建的用户和后续 OIDC 登录能正确关联到同一个 Teable 用户。
   * 约定：凡经此路径创建的用户永远不是超级管理员，不参与「首个账户即管理员」逻辑。
   */
  async getOrCreateUserByProvider(
    ...
    // 优先按 (provider, providerId) 在 account 表中查找
    const existAccount = await this.prismaService.txClient().account.findFirst({
      where: { provider, providerId },
    });
    if (existAccount) {
      const user = await this.getUserById(existAccount.userId);
      if (!user) {
        throw new Error(
          `account(${provider}/${providerId}) 关联的用户 ${existAccount.userId} 不存在`
        );
      }
      return { id: user.id, email: user.email, name: user.name };
    }

    // account 不存在，按 email 查找是否有已存在的用户
    const existUser = await this.getUserByEmail(normalizedEmail);
    if (existUser) {
      // 用户已存在但无此 provider 的 account，创建 account 关联
      await this.prismaService.txClient().account.create({
        data: {
    ...
    // 用户和 account 均不存在，同时创建（不赋予超级管理员，仅普通用户）
    const newUser = await this.createUser(
      ...
      { provider, providerId, type: 'oauth' },
      undefined,
      false,
      false
    );
  ...
  async findOrCreateUser(
    ...
    const res = await this.prismaService.$tx(async () => {
      ...
      if (!existUser) {
        ...
        isNewUser = true;
        onCreateNewUser?.();
        // OAuth/SSO 用户应绕过禁止注册检查
        const isOAuthUser = type === 'oauth';
