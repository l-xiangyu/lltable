...
export class OauthStoreService {
  ...
  async store(req: Request, callback: (err: unknown, stateId: string) => void, ...args: unknown[]) {
    ...
    const stateData: IOauth2State = {
      redirectUri: req.query.redirect_uri as string,
    };
    if (req.query.visitId) {
      stateData.visitId = req.query.visitId as string;
    }
    await this.cacheService.set(`oauth2:${random}`, stateData, second('12h'));
  ...
  async verify(
    ...
    if (state) {
      ...
      if (state.visitId) {
        (req as any)._oidcVisitId = state.visitId;
      }
