...
export class SqliteRecordQueryDialect implements IRecordQueryDialectProvider {
  ...
  selectUserNameById(idRef: string): string {
    return `COALESCE((SELECT name FROM users WHERE id = ${idRef}), ${idRef})`;
  }

  buildUserJsonObjectById(idRef: string): string {
    return `json_object(
        'id', ${idRef},
        'title', COALESCE((SELECT name FROM users WHERE id = ${idRef}), ${idRef}),
        'email', (SELECT email FROM users WHERE id = ${idRef})
      )`;
  }

  buildAuditUserJsonFromPersistedColumn(
    persistedColumnRef: string,
    systemUserIdRef: string
  ): string {
    return `COALESCE(
        CASE
          WHEN ${persistedColumnRef} IS NOT NULL
            AND json_type(${persistedColumnRef}) = 'object'
            AND NULLIF(json_extract(${persistedColumnRef}, '$.title'), '') IS NOT NULL
            AND json_extract(${persistedColumnRef}, '$.title') != json_extract(${persistedColumnRef}, '$.id')
          THEN ${persistedColumnRef}
          ELSE NULL
        END,
        ${this.buildUserJsonObjectById(systemUserIdRef)}
      )`;
  }
