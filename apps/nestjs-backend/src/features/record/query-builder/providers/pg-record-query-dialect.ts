...
export class PgRecordQueryDialect implements IRecordQueryDialectProvider {
  ...
  selectUserNameById(idRef: string): string {
    return `COALESCE((SELECT u.name FROM users u WHERE u.id = ${idRef}), ${idRef}::text)`;
  }
  ...
  buildUserJsonObjectById(idRef: string): string {
    return `COALESCE(
        ...
        jsonb_build_object('id', ${idRef}, 'title', ${idRef}::text)
      )`;
  }
  ...
  buildAuditUserJsonFromPersistedColumn(
    ...
    const persistedJson = `${persistedColumnRef}::jsonb`;
    return `COALESCE(
        CASE
          WHEN ${persistedColumnRef} IS NOT NULL
            AND jsonb_typeof(${persistedJson}) = 'object'
            AND NULLIF(${persistedJson}->>'title', '') IS NOT NULL
            AND (${persistedJson}->>'title') IS DISTINCT FROM (${persistedJson}->>'id')
          THEN ${persistedJson}
          ELSE NULL
        END,
