...
import { JbsAuditUserService } from '../user/jbs-audit-user.service';
...
export class RecordService {
  ...
  constructor(
    ...
    private readonly jbsAuditUserService: JbsAuditUserService
  ) {}
  ...
  async buildFilterSortQuery(
    ...
    // 筛选「当前用户」时：OIDC 嵌入场景用主端 userId，否则用 Teable userId
    const currentUserId = await this.getFilterCurrentUserId();
  ...
  async createRecordsOnlySql(
    ...
    const { auditUserId, auditUserValue } = await this.jbsAuditUserService.getAuditContext();
  ...
  private async createBatch(
    ...
    const sanitizeAuditUserValue = () =>
      this.jbsAuditUserService.sanitizeAuditUserValue(cloneAuditUserValue());
    ...
    const snapshots = records
      ...
      .map((order, i) => {
        ...
        return removeUndefined({
          __id: snapshot.id,
          __created_by: snapshot.createdBy || auditUserId,
  ...
  private async getFilterCurrentUserId(): Promise<string> {
    const teableUserId = this.cls.get('user.id');
    if (!teableUserId) {
      return '';
    }
    return this.jbsAuditUserService.resolveAuditUserId(teableUserId);
  }
  ...
  async getRowCountByFilter(
    ...
    const withUserId = await this.getFilterCurrentUserId();
