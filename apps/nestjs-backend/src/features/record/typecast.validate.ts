...
import { isJbsMainUserId } from '../../utils/jbs-user-id.util';
      ...
      (validatedCellValue: unknown) => {
        if (this.field.isMultipleCellValue) {
          return (validatedCellValue as IUserCellValue[]).map((v) => {
            // 嵌入场景：主端用户 ID 不查 Teable 协作者表
            if (isJbsMainUserId(v.id)) {
              return UserFieldDto.fullAvatarUrl(v);
            }
            const user = userMap[v.id];
            if (!user) {
              throw new CustomHttpException(
                `User(${v.id}) not found in table(${this.tableId})`,
                HttpErrorCode.VALIDATION_ERROR,
                {
                  localization: {
        ...
        const cellUser = validatedCellValue as IUserCellValue;
        if (isJbsMainUserId(cellUser.id)) {
          return UserFieldDto.fullAvatarUrl(cellUser);
        }
        const user = userMap[cellUser.id];
        if (!user) {
          throw new CustomHttpException(
            `User(${cellUser.id}) not found in table(${this.tableId})`,
            HttpErrorCode.VALIDATION_ERROR,
            {
              localization: {
                i18nKey: 'httpErrors.user.notFound',
