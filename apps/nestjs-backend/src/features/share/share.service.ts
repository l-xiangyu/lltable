...
export class ShareService {
  ...
  /**
   * 分享视图中更新记录，校验编辑权限及记录/字段可见性后委托 recordOpenApi；分享开启 allowEdit 后可在线编辑已有记录
   * 26年4月17日修改分享视图权限
   */
  async updateRecord(shareInfo: IShareViewInfo, recordId: string, updateRecordRo: IUpdateRecordRo) {
    const { tableId, shareMeta } = shareInfo;
    // 校验分享元信息是否允许编辑
    if (!shareMeta?.allowEdit) {
      throw new CustomHttpException('not allowed to edit', HttpErrorCode.RESTRICTED_RESOURCE, {
        localization: {
          i18nKey: 'httpErrors.share.notAllowedToEdit',
        },
      });
    }
    ...
    // 校验提交的字段是否在分享视图可见范围内
    const fieldIds = Object.keys(updateRecordRo.record.fields);
    await this.shareSocketService.validFieldSnapshotPermission(shareInfo, fieldIds);

    return this.recordOpenApiService.updateRecord(tableId, recordId, updateRecordRo);
  }

  /**
   * 分享视图中新增记录，校验编辑权限后委托 recordOpenApi；分享开启 allowEdit 后可新增行
   * 26年4月17日修改分享视图权限
   */
  async createRecords(shareInfo: IShareViewInfo, createRecordsRo: ICreateRecordsRo) {
    const { tableId, shareMeta } = shareInfo;
    // 校验分享元信息是否允许编辑
    if (!shareMeta?.allowEdit) {
      throw new CustomHttpException('not allowed to edit', HttpErrorCode.RESTRICTED_RESOURCE, {
