...
export class ShareController {
  ...
  /**
   * 分享视图更新单条记录；新增 PATCH 供分享页编辑调用
   * 26年4月17日修改分享视图权限
   */
  @UseGuards(ShareAuthGuard)
  @Patch('/:shareId/view/record/:recordId')
  async updateRecord(
    @Request() req: any,
    ...
    const shareInfo = req.shareInfo as IShareViewInfo;
    return this.shareService.updateRecord(shareInfo, recordId, updateRecordRo);
  }

  /**
   * 分享视图批量新增记录；新增 POST 供分享页新增行调用
   * 26年4月17日修改分享视图权限
   */
  @UseGuards(ShareAuthGuard)
  @Post('/:shareId/view/records')
  async createRecords(
    @Request() req: any,
    @Body(new ZodValidationPipe(createRecordsRoSchema)) createRecordsRo: ICreateRecordsRo
  ): Promise<ICreateRecordsVo> {
    const shareInfo = req.shareInfo as IShareViewInfo;
    return this.shareService.createRecords(shareInfo, createRecordsRo);
  }
