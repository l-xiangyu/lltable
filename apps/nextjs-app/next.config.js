...
const nextConfig = {
  ...
  async rewrites() {
    ...
    // 管理后台「AI 配置」入口指向同一设置页（无独立 ai-setting 页面时）
    const adminAiSettingRewrite = {
      source: '/admin/ai-setting',
      destination: '/admin/setting',
    };

    return isProd ? [adminAiSettingRewrite] : [adminAiSettingRewrite, socketProxy];
  },
