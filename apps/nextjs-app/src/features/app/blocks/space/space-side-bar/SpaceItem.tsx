...
export const SpaceItem: React.FC<IProps> = ({ space, isActive }) => {
  ...
  // 仅超级管理员可修改空间名称 2025-03-09
  const canRenameSpace = Boolean(user?.isAdmin);
    ...
    <div className="relative overflow-y-auto">
      <ItemButton className="group" isActive={isActive} ref={ref}>
        <Link
          ...
          onDoubleClick={() => canRenameSpace && setIsEditing(true)}
        >
          ...
          <SpaceOperation
            space={space}
            onRename={canRenameSpace ? () => setIsEditing(true) : undefined}
            open={open}
            setOpen={setOpen}
