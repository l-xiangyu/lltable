...
export const KanbanCard = (props: IKanbanCardProps) => {
  ...
  const { createRecords, deleteRecord, duplicateRecord } = useRecordOperations();
  ...
  const onDelete = async () => {
    ...
    await deleteRecord({ tableId, recordId: card.id });
  };
  ...
  const onDuplicate = async () => {
    ...
    await duplicateRecord({
      tableId,
      recordId: card.id,
      order: { viewId, anchorId: card.id, position: 'after' },
    });
  };
