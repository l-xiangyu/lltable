...
import { stopFillField, UploadType, autoFillCell } from '@teable/openapi';
import type {
  IRectangle,
  IPosition,
  IGridRef,
...
import { useJbsReferenceRecords, isJbsReferenceFieldEditable } from '@teable/sdk/context';
import {
  useBaseId,
  useFields,
  useIsTouchDevice,
...
import { JBS_REFERENCE_FIELD_NOT_EDITABLE_MESSAGE } from '@/features/embed/jbs-reference-messages';
...
export const GridViewBaseInner: React.FC<IGridViewBaseInnerProps> = (
  ...
  const { updateRecord, duplicateRecord, createRecords } = useRecordOperations();
  ...
  // 切换表（含首次进入）时清空 Grid 行缓存，避免引用表/普通表切换后 recordMap 残留
  useEffect(() => {
    onReset();
  }, [onReset, tableId]);
  ...
  const onDeleteForPrefilling = (selection: CombinedSelection) => {
    ...
    for (let col = startCol; col <= endCol; col++) {
      const fieldId = columns[col]?.id;
      if (!fieldId) continue;
      if (!isJbsReferenceFieldEditable(jbsReference, fieldId)) {
        continue;
      }
  ...
  const onCellDblClick = (cell: ICellItem) => {
    ...
    if (record.isLocked(field.id)) {
      if (
        ...
        jbsReference.fieldMetas?.find((meta) => meta.fieldId === field.id)?.isEditable === false
      ) {
        return sonnerToast.warning(JBS_REFERENCE_FIELD_NOT_EDITABLE_MESSAGE);
      }
