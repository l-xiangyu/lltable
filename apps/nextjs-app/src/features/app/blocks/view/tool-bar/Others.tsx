...
import { SharePublishMenuButton } from './SharePublishMenuButton';
...
const OthersList = ({
  ...
  return (
    <div className={cn('gap-1 flex items-center', className)}>
      <div className={cn('flex items-center gap-1', foldButton && 'w-full flex-col items-stretch')}>
        <SharePopover>
          {(text, isActive) => (
            <ToolBarButton
              isActive={isActive}
        ...
        <SharePublishMenuButton className={cn(foldButton && 'w-full shrink-0 font-normal')} />
      </div>
