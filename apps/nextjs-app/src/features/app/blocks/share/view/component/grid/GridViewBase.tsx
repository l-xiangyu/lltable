...
import type { HttpError, IGridViewOptions } from '@teable/core';
import { FieldKeyType, RowHeightLevel } from '@teable/core';
import type { IGetRecordsRo, IGroupPointsVo, IRangesRo } from '@teable/openapi';
import {
  saveQueryParams,
  shareViewCopy,
  shareViewUpdateRecord,
  shareViewCreateRecords,
} from '@teable/openapi';
import type {
  CombinedSelection,
  IButtonCell,
...
import { getHttpErrorMessage } from '@teable/sdk/context';
import {
...
export const GridViewBase = (props: IGridViewProps) => {
  ...
  // 分享页单元格编辑：乐观更新后调接口，失败回滚并提示国际化错误；allowEdit 开启后可用；26年4月17日修改分享视图权限
  const onCellEdited = useCallback(
    (cell: ICellItem, newVal: IInnerCell) => {
      ...
      record.fields[fieldId] = newCellValue;

      shareViewUpdateRecord(shareId, record.id, {
        fieldKeyType: FieldKeyType.Id,
        record: {
          fields: {
            [fieldId]: newCellValue === undefined ? null : newCellValue,
          },
        },
      }).catch((e) => {
        record.fields[fieldId] = oldCellValue;
        const message = getHttpErrorMessage(e as HttpError, t, 'sdk');
        toast.error(message);
      });
    },
    [recordMap, columns, router.query.shareId, t]
  );

  // 分享页底部新增空行；allowEdit 开启后显示新增；26年4月17日修改分享视图权限
  const onRowAppend = useCallback(() => {
    const shareId = router.query.shareId as string;
    shareViewCreateRecords(shareId, {
      fieldKeyType: FieldKeyType.Id,
      records: [{ fields: {} }],
    }).catch((e) => {
      const message = getHttpErrorMessage(e as HttpError, t, 'sdk');
      ...
      {prepare ? (
        <>
          <Grid
            ...
            onCopy={onCopy}
            // 按 allowEdit 决定是否挂载编辑与新增；26年4月17日修改分享视图权限
            onCellEdited={view?.shareMeta?.allowEdit ? onCellEdited : undefined}
            onRowAppend={view?.shareMeta?.allowEdit ? onRowAppend : undefined}
