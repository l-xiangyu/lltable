...
import { useBaseId, useBasePermission, useSession, useTables } from '@teable/sdk/hooks';
...
/** 安全触发删除：吞掉 async rejection，避免 Next.js 开发模式弹出 Runtime Error */
const invokeDeleteSafely = (
  onDelete: IBaseNodeMoreProps['onDelete'],
  permanent: boolean,
  confirm?: boolean
) => {
  if (!onDelete) return;
  void onDelete(permanent, confirm).catch(() => undefined);
};
      ...
      <>
        ...
        {canPermanentDelete && (
          <ListMenuItem
            ...
            onClick={() => invokeDeleteSafely(onDelete, true)}
            destructive
          />
        )}
        {canDelete && (
          <ListMenuItem
            ...
            onClick={() => invokeDeleteSafely(onDelete, false)}
            destructive
          />
        )}
      </>
...
export const TableOperation = (props: IBaseNodeMoreProps) => {
  ...
  // 仅超级管理员可见API入口 2026-02-28 00:00:00
