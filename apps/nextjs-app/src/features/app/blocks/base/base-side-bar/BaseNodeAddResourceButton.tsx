...
import { Database, FileCsv, FileExcel, Slack, Table2 } from '@teable/icons';
...
import { CreateTableFromIndicatorLibraryDialog } from '@/features/embed/CreateTableFromIndicatorLibraryDialog';
import { isInLltableEmbed } from '@/features/embed/lltable-parent-bridge';
...
export const BaseNodeAddResourceButton = (props: BaseNodeAddResourceButtonProps) => {
  ...
  const AddTableMenuItems = () => {
    ...
    return (
      <>
        ...
        {isInLltableEmbed() && (
          <DropdownMenuItem className="cursor-pointer" onClick={() => setIndicatorDialogOpen(true)}>
            <Button variant="ghost" size="xs" className="h-4">
              <Database className="size-4" />
              {t('table:table.operator.createFromIndicatorLibrary', {
                defaultValue: '从指标库新建表格',
              })}
            </Button>
          </DropdownMenuItem>
        )}
  ...
  return (
    <div className="flex w-full flex-col">
      ...
      {isInLltableEmbed() && (
        <CreateTableFromIndicatorLibraryDialog
