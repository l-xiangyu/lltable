...
export const getTableServerSideProps = async (
  ...
  // 与布局一致：?isExternal=true 时隐藏侧栏等（避免仅靠客户端 query 导致 SSR/CSR 不一致），26年4月22日
  const rawExternal = queryParams?.isExternal;
  const isExternalEmbed =
    rawExternal === 'true' || (Array.isArray(rawExternal) && rawExternal[0] === 'true');
  return {
    props: {
      ...serverData,
